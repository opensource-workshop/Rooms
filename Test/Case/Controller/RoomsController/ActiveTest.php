...
/**
 * active()アクションのテスト(GETのテスト、JSON形式)
 *
 * @return void
 */
	public function testActiveGetJson() {
		//テスト実行
		$this->_testGetAction(array('action' => 'active', '2', '4'), null, 'BadRequestException', 'json');
	}
...
/**
 * active()アクションのテスト(POSTのテスト、Saveエラー、JSON形式)
 *
 * @return void
 */
	public function testActivePostSaveErrorJson() {
		$this->_mockForReturnFalse('Rooms.Room', 'saveActive');

		//テスト実行
		$data = array('Room' => array('id' => '4'));
		$this->_testPostAction('put', $data, array('action' => 'active', '2', '4'), 'BadRequestException', 'json');
	}
